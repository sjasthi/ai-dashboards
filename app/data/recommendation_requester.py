...
"pattern_used" MUST be exactly one of those six values, letter-for-letter - never a
free-text label like "Join and Aggregate" or "Multi-File Analysis". A join is a data-prep
step declared in "required_operations", not a pattern of its own: if a report needs to
join two files first, that join happens in required_operations, and the report's
pattern_used is still whichever of the six matches what the FINAL joined/aggregated
result shows (e.g. a top-N ranking after joining -> RANKING; a breakdown by category
after joining -> COMPOSITION).
