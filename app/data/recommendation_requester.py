...
2g. Every "files_involved" entry MUST be copied character-for-character from that file's "filename" field above - never shorten, rename or guess one. This matters most when a file's sheets were disambiguated as "sheet (workbook).xlsx": if a data_dictionary/schema sheet in the same upload describes a table as plain "accounts" or "accounts.xlsx", that is documentation of the real-world table, not the filename to cite - still use the exact disambiguated "filename" (e.g. "accounts (workbook).xlsx") from that file's own profile.
